...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
        ...
        if(!empty($variable)){
            ...
            $sql = "select *, match (nombre, nacionalidad) against ('".$variable."'') ";
            $sql.= "from ".TABLE_PREFIX._TAB_RVOZ_SYS_." ";
            $sql.= "where match (nombre, nacionalidad) against ('".$variable."'') ";
            ...
            if($encontro == 0){
               ...
               if(count($row)==0){
                  $sql = "select * ";
                  $sql.= "from ".TABLE_PREFIX._TAB_RVOZ_SYS_." ";
                  $sql.= "where nombre like '%".$variable."%' or nacionalidad like '%".$variable."%' ";
                  $sql.= "order by nombre asc";
                  $row = $conectdata->set_list_table($sql); 
               }
            }
            ...
            foreach($row as $key => $valor){
                echo '<option value="'.$row[$key]['idv'].'">'.utf8_encode($row[$key]['nombre']).' ('.utf8_encode($row[$key]['nacionalidad']).')</option>';
                $encontro++;
            }
